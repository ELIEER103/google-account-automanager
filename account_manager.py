...
class AccountManager:
    ...
    @staticmethod
    def save_bind_link(line_or_email: str, bind_url: str):
        """保存绑卡链接到文件"""
        if not bind_url:
            return
        email, pwd, rec, sec, _link = AccountManager._parse(line_or_email)
        if not email and '@' in line_or_email:
            email = line_or_email

        if not email:
            print("[AM] save_bind_link: 无法解析邮箱，跳过")
            return

        line = f"{bind_url}----{email}"
        if pwd:
            line += f"----{pwd}"
        if rec:
            line += f"----{rec}"
        ...
        path = os.path.join(base_path, "绑定卡链接.txt")
        try:
            with open(path, 'a', encoding='utf-8') as f:
                f.write(line + "\n")
            print(f"[AM] 已保存绑卡链接: {email}")
        except Exception as e:
            print(f"[AM] 保存绑卡链接失败: {e}")
